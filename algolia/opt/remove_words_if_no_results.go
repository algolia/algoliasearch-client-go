// Code generated by go generate. DO NOT EDIT.

package opt

import "encoding/json"

// RemoveWordsIfNoResultsOption is a wrapper for an RemoveWordsIfNoResults option parameter. It holds
// the actual value of the option that can be accessed by calling Get.
type RemoveWordsIfNoResultsOption struct {
	value string
}

// RemoveWordsIfNoResults wraps the given value into a RemoveWordsIfNoResultsOption.
func RemoveWordsIfNoResults(v string) *RemoveWordsIfNoResultsOption {
	return &RemoveWordsIfNoResultsOption{v}
}

// Get retrieves the actual value of the option parameter.
func (o *RemoveWordsIfNoResultsOption) Get() string {
	if o == nil {
		return "none"
	}
	return o.value
}

// MarshalJSON implements the json.Marshaler interface for
// RemoveWordsIfNoResultsOption.
func (o RemoveWordsIfNoResultsOption) MarshalJSON() ([]byte, error) {
	return json.Marshal(o.value)
}

// UnmarshalJSON implements the json.Unmarshaler interface for
// RemoveWordsIfNoResultsOption.
func (o *RemoveWordsIfNoResultsOption) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		o.value = "none"
		return nil
	}
	return json.Unmarshal(data, &o.value)
}

// Equal returns true if the given option is equal to the instance one. In case
// the given option is nil, we checked the instance one is set to the default
// value of the option.
func (o *RemoveWordsIfNoResultsOption) Equal(o2 *RemoveWordsIfNoResultsOption) bool {
	if o == nil {
		return o2 == nil || o2.value == "none"
	}
	if o2 == nil {
		return o == nil || o.value == "none"
	}
	return o.value == o2.value
}

// RemoveWordsIfNoResultsEqual returns true if the two options are equal.
// In case of one option being nil, the value of the other must be nil as well
// or be set to the default value of this option.
func RemoveWordsIfNoResultsEqual(o1, o2 *RemoveWordsIfNoResultsOption) bool {
	return o1.Equal(o2)
}
