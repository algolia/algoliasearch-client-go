...
func NewClientWithConfig(cfg PersonalizationConfiguration) (*APIClient, error) {
	if cfg.AppID == "" {
...
func getDefaultHosts(r Region) []transport.StatefulHost {
	return []transport.StatefulHost{transport.NewStatefulHost("https", strings.ReplaceAll("personalization.{region}.algolia.com", "{region}", string(r)), call.IsReadWrite)}
}

func getUserAgent() string {
	return fmt.Sprintf("Algolia for Go (4.0.0-beta.29); Go (%s); Personalization (4.0.0-beta.29)", runtime.Version())
}
...
func (c *APIClient) GetConfiguration() *PersonalizationConfiguration {
	return c.cfg
}
